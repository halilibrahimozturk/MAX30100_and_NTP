...
extern Semaphore_Handle semaphore4;
...
//        System_printf("Counter %d\n", ctr);
        time  = timeStamp[0]*16777216 +  timeStamp[1]*65536 + timeStamp[2]*256 + timeStamp[3];
        time += 10800;
        time += ctr++;
        System_printf("Time : %s", ctime(&time));
        System_flush();
...
//                System_flush();
              ...
              // DC FILTERING
                      dcFiltered = input + (0.95 * filteredPrevious);
...
Void ntpSocket(UArg arg0, UArg arg1)
{
        Semaphore_pend(semaphore4, BIOS_WAIT_FOREVER);
        getTimeWithNTP("128.138.140.44", 37,time, strlen(time));
}
...
bool createTasks(void)
{
    ...
    taskParams.stackSize = TASKSTACKSIZE;
    taskParams.priority = 1;
    taskHandle5 = Task_create((Task_FuncPtr)ntpSocket, &taskParams, &eb);
...
void getTimeWithNTP(char *serverIP, int serverPort, char *data, int size)
{
        ...
        tri = recv(sockfd, timeStamp, sizeof(timeStamp), 0);
        if(tri < 0) {
